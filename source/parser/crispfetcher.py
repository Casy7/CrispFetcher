		...
		while False:
			...
			if type(xml_1[old_obj_index]) == XMLLine:
				...
				if this_item_in_new_xml:
					...
					new_xml_2.append(new_item)
					new_xml_2_lines += curr_item.length()
...
xml_file = XMLToStructureComposer("", "")
...
@eel.expose
def send_old_xml():
	return [obj.__dict__ for obj in xml_file.new_old_XML_groups]


@eel.expose
def send_new_xml():
	return [obj.__dict__ for obj in xml_file.new_new_XML_groups]


@eel.expose
def send_res_xml():
	return [obj.__dict__ for obj in xml_file.res_XML_groups]
